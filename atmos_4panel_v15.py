#!/usr/bin/env python3
"""Animate a four-panel analysis of the ten newest KLOT radar volumes.

Designed to run directly from Spyder without command-line arguments. The
script finds the ten newest complete KLOT NEXRAD Level-II volumes in NOAA's
public Amazon S3 archive, reads them remotely without retaining local radar
files, plots regional and site-scale reflectivity, builds a pseudo-RHI from
KLOT toward ATMOS, blends a Doppler-velocity column above ATMOS with the KLOT
VAD cross-beam wind component, estimates the instantaneous rain rate near
ATMOS, overlays NWS warning polygons valid at each scan time, and saves the
sequence as one looping GIF.

Required packages (conda-forge):
    arm_pyart cartopy matplotlib metpy netcdf4 numpy s3fs

Important:
    A single radar volume provides an instantaneous rain-rate estimate, not a
    storm-total precipitation accumulation. Accumulation requires multiple
    consecutive volume scans and integration through time.
"""
